<template>
    
    <section class="container mx-0 mt-10">

     <div class="container mb-10">
    
      <div class="text-5xl text-center font-tajawal ">
            <p>  من نحن ؟  </p>
          </div>
     
     </div>


     <div class="flex">
    <div class="w-2/3 flex flex-col space-y-4 font-tajawal px-6 ml-20 bg-gradient-to-b from-neutral-900 to-black">
      <p class="font-tajawal text-4xl	">
مسلم بقيمة مضافة      </p>
      <p>
        صفحة تقنية يُديرها شباب جزائري واعد، يريد الإسهام في بناء مجتمع تعلمي يواكب التغيرات في المجال التّقني و التكنولوجي بصفة عامة!
⚡️ المجالات التي سنهتم بها: 
- البرمجة: من تطبيقات الهاتف، تطبيقات سطح المكتب، الواب و الألعاب
- الذكاء الاصطناعي، الأنظمة المعلوماتية، الأمن المعلوماتي، تحليل البيانات!
- الطباعة ثلاثية الأبعاد.
- الكترونيك.
- الهاردوير و كل ما يتعلق بقطع الحواسيب و الهواتف.
- أخبار تقنية، مصادر تعليمية.
- وكل ما يحتاجه الطالب من محتوى علمي، بحوث ومذكرات.
            </p>
      
    </div>
    
    <div class="w-0 border-l border-transparent"></div>
    <!-- Left card -->
    <div class="w-1/2 p-6 font-tajawal">
      <div class="grid grid-cols-2 gap-2">
        <div >
          <a href="#" class="block max-w-sm p-6 bg-black border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">

          <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-orange_atm">01</h5>
          <p class="font-normal text-gray-700 dark:text-gray-400">مسلم بقيمة مضافة  </p>
          </a>
        </div>
        <div >
          <a href="#" class="block max-w-sm p-6 bg-black border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">

          <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-orange_atm">02</h5>
          <p class="font-normal text-gray-700 dark:text-gray-400">مسلم بقيمة مضافة  </p>
          </a>
        </div>
        <div >
          <a href="#" class="block max-w-sm p-6 bg-black border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">

          <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-orange_atm">03</h5>
          <p class="font-normal text-gray-700 dark:text-gray-400">مسلم بقيمة مضافة  .</p>
          </a>
        </div>
        <div >
          <a href="#" class="block max-w-sm p-6 bg-black border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">

          <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-orange_atm">04</h5>
          <p class="font-normal text-gray-700 dark:text-gray-400">مسلم بقيمة مضافة  </p>
          </a>
        </div>
    </div>
    </div>

   
  </div>
 

    </section>
</template>

<script>

 
</script>

<style scoped>

</style>